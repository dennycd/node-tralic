// -*- C++ -*-
// $Id: txparam.h,v 2.43 2010/03/31 08:30:15 grimm Exp $
...
class MainClass
{
  ...
  bool have_dir; // true if dir switch given.
  bool use_tcf;
  bool hacknotitle;
  bool etex_enabled;
...
public:
  ...
  bool get_hacknotitle() { return hacknotitle; }
  ...
  void parse_option(int&,int,const char* argv[]);
  void banner();
  String print_os() const;
  void read_config_and_other();
  void RRbib(String);
  void run(); 
  void run(int n, const char* argv[]);
...
private:
  ...
  String check_for_arg(int&p, int argc,const char* argv[]);
