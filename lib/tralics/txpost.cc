...
const char* txpost_rcsid= 
	    "$Id: txpost.C,v 2.69 2011/06/28 08:15:25 grimm Exp $";
...
void Xml::convert_to_string_safe(string& ss) const
{
    //if a text node, simply append the text
    string tagname = name.c_str();
    if(is_xmlc()){
        ss += tagname;
        return;
    }

    //enclose xml being tag
    ostringstream oss;
    oss << "<" << tagname; //">";
    
    //append math tag if see
    if(tagname=="math")
        oss << " mode=\"display\" xmlns=\"http://www.w3.org/1998/Math/MathML\"";
    oss << ">";
    ss += oss.str();
    
    //descend 
    for(int k=0; k<size();k++)
        tree[k]->convert_to_string_safe(ss);
